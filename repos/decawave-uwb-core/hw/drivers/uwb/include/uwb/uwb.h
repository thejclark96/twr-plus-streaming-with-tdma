...
/**
 * Read receive time. (As adjusted by the LDE) (Leading Edge Detection)
 *
 * @param inst  Pointer to struct uwb_dev.
 *
 * @return time
...
/**
 * Read STS receive time. (As adjusted by the LDE) (scrambled timestamp sequence (STS))
 *
 * @param inst  Pointer to struct uwb_dev.
 *
 ...
 * @param nlen      The length of the frame to be transmitted/received excluding crc (cyclic redundancy check)
...
/**
 * API to calculate the SHR (synchronization header) (Preamble + SFD) duration. This is used to calculate the correct rx_timeout.
...
/**
 * Calculate rssi (Received Signal Strength) from last RX in dBm.
