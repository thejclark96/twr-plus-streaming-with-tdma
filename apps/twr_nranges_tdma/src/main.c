...
int uwb_config_updated_cb()
{
    ...
    {                                              // Checks if there are no CCP semaphores
...
struct uwbcfg_cbs uwb_cb = {
    // This block of code seems to appear in every app. I can't figure out why. Nevermind, look at the comment below.
...
static bool complete_cb(struct uwb_dev *inst, struct uwb_mac_interface *cbs)
{
    ...
    { // Checks if received frame is an IEEE range frame
        return false;
    }
...
{               // ev = os_event = dpl_event (dpl = Decawave Porting Layer)
    ...
    if (ccp->local_epoch == 0 || udev->slot_id == 0xffff)
        return; // If ccp's epoch==0 AND slot ID is all 1s, return

    /* Process any newtmgr packages queued up */
    if (idx > 6 && idx < (tdma->nslots - 6) && (idx % 4) == 0)
    { // if tdma slot # is greater than 6 AND less than the (total slots - 6) AND divisible by 4:
    ...
    { // If device is NOT an ANCHOR
        /* Range with the anchors */
        if (idx % MYNEWT_VAL(NRNG_NTAGS) != udev->slot_id)
        {           // NRNG_NTAGS = Max number of tags to allow in slots | slot_id =  Assigned slot_id
